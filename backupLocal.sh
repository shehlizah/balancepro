...
mkdir -p "$MAIN_BACKUP_FOLDER/public_html/includes/core"
mkdir -p "$WL_BACKUP_FOLDER/public_html/includes/core"
...
mkdir -p "$MAIN_BACKUP_FOLDER/public_html/wp-content/themes/balance-theme/css"
mkdir -p "$MAIN_BACKUP_FOLDER/public_html/wp-content/themes/balance-theme/inc/renderers/modules"

# Copy files to the correct destination directories
cp "$RENDER_DIR" "$WL_BACKUP_FOLDER/public_html/templates/"
cp "$CSS_DIR" "$WL_BACKUP_FOLDER/public_html/assets/css/"
cp "$PG_STG1" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$PG_CHECK" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$PG_LS" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$PG_NEW_CHECK" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$PG_SORT_CHECK" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$PG_TAGS_NCHECK" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$PG_BTN_LS" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$PG_BTN_TAGS" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$PGN_BTN_RS" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$RS_ALL" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$RS_LS" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$RS_PG_ALL_NEW" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$RS_PG_BACK_CONTENT" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$RS_SEARCH" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$RS_SEARCH_NEW" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$RS_SORT_SEARCH" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$RS_TAG_SEARCH" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$SEARCH_QUERY" "$WL_BACKUP_FOLDER/public_html/includes/core/"
cp "$WL_SQP" "$WL_BACKUP_FOLDER/public_html/includes/core/"

# Main website files copying
cp "$MAIN_CSS" "$MAIN_BACKUP_FOLDER/public_html/wp-content/themes/balance-theme/css/"
cp "$RENDER_DIR_MAIN" "$MAIN_BACKUP_FOLDER/public_html/wp-content/themes/balance-theme/inc/renderers/modules/"
cp "$MAIN_PG_STG1" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_PG_CHECK" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_PG_LS" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_PG_NEW_CHECK" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_PG_BTN_LS" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_PG_BTN_NEW_LS" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_PG_BTN_TAGS" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_PGN_BTN_RS" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_RS_ALL" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_RS_LS" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_RS_PG_ALL_NEW" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_RS_PG_BACK_CONTENT" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_RS_SEARCH_PG_CONTENT" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_RS_SEARCH_NEW" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_RS_SORT_SEARCH" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_RS_TAG_SEARCH" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_RS_TAGS" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_RS_PG_CONTENT" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_RS_SEARCH" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_SEARCH_QUERY" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
cp "$MAIN_SQ_PG" "$MAIN_BACKUP_FOLDER/public_html/includes/core/"
