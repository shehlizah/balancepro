...
<?php
if($_GET['action']=="GettingStarted"){
                $lcontent = '50';
        } else if($_GET['action']=="NewBeginnings"){
                $lcontent = '183';
        } else if($_GET['action']=="LifesCurveballs"){
                $lcontent = '184';
        } else if($_GET['action']=="ManagingDebt"){
                $lcontent = '185';
        } else if($_GET['action']=="Retirement"){
                $lcontent = '186';
        }


$getlsContentData = "SELECT * FROM wp_postmeta WHERE post_id = '$lcontent' AND meta_key = '_page_edit_data'";

                $lsContentResults = $db->prepare($getlsContentData);

                        $lsContentResults->execute();
                        $lsContentResult[]=$lsContentResults->fetchAll();
$lsPostmetaData = unserialize($lsContentResult[0][0]['meta_value']);
$bannerImage = $lsPostmetaData['m1_module'][0]['bgimage']['image']['fullpath'];
$bannerTitle = $lsPostmetaData['m1_module'][0]['title'];
$bannerCopy = $lsPostmetaData['m1_module'][0]['copy'];

?>
<p style="margin-top: 139px;"></p>

<div style="background-image: url(<?php echo $bannerImage;?>);" class="newbanner banner text-center">
        <div class="container">
                <div class="row">
...
$contentTitle = $lsPostmetaData['m31_module'][1]['title'];
$contentCopy = $lsPostmetaData['m31_module'][1]['copy'];

?>
<article name="" class="text-block article default-content-style" aria-label="module for article <?php echo $contentTitle;?>">
        <div class="container">
            <div class="row">
                <h1 class="text-info text-center"><?php echo $contentTitle;?></h1>
                <?php echo $contentCopy;?>
        </div>
    </div>
</article>
                                        ...
                                        $table_name = "wp_term_relationships";
									     $table_name2 = "wp_terms";
							                    ...
							                    <!-- resource in resources starts -->
							                              <div class="col-sm-6 col-md-4"><a href="<?php echo $viewUrl; ?>" target="_self">
