...
// if search is 0 and life stage is 0 and resourcetype is 0 starts
    ...
    if($tags !='0'){
        ...
        $query = "SELECT Distinct * FROM wp_resources as w, wp_posts AS P WHERE w.status = 'publish' AND w.level_of_access = '$level' AND P.ID = w.wp_post_id AND w.list_in_search = '$list' AND status='publish'  ORDER BY P.post_date_gmt DESC limit $start, $limit";
         $result = $db->prepare($query);
    }
}   
// if search is 0 and life stage is 0 and resourcetype is 0 ends
    ...
    if($sort == '0'){
        if($tags !='0'){
        $query = "SELECT Distinct * FROM wp_resources as w, wp_term_relationships as wtr WHERE w.status = 'publish' AND w.type = '$dtype' AND w.level_of_access = '$level' AND w.list_in_search = '$list' AND w.page_order = '$pgorder'  AND  w.wp_post_id = wtr.object_id AND wtr.term_taxonomy_id IN ('$advancedkeywords')  limit $start, $limit";
        $result = $db->prepare($query);
    } else {
        ...
        if($tags !== '0'){
            $query = "SELECT Distinct * FROM wp_resources as w, wp_term_relationships as wtr WHERE w.status = 'publish' AND w.type = '$dtype' AND w.level_of_access = '$level' AND w.list_in_search = '$list' AND w.page_order = '$pgorder'  AND  w.wp_post_id = wtr.object_id AND status='publish' AND wtr.term_taxonomy_id IN ('$advancedkeywords') limit $start, $limit";
            $result = $db->prepare($query);
        } else {
            $query = "SELECT Distinct * FROM wp_resources as w, wp_term_relationships as wtr WHERE w.status = 'publish' AND w.type = '$dtype' AND w.level_of_access = '$level' AND w.list_in_search = '$list' AND w.page_order = '$pgorder' limit $start, $limit";
            $result = $db->prepare($query);
        }
        ...
        if($tags !== '0'){
            ...
            $query = "SELECT Distinct * FROM wp_resources as w, wp_resources_view_count as wc WHERE w.status = 'publish' AND w.type = '$dtype' AND w.level_of_access = '$level' AND w.list_in_search = '$list'  AND wc.wp_post_id = w.wp_post_id AND w.page_order = '$pgorder'  ORDER BY wc.view_count DESC limit $start, $limit";
        $result = $db->prepare($query); 
        }
       
    } else if($sort=='date'){
        if($tags !== '0'){
            $query = "SELECT Distinct * FROM wp_resources as w, wp_posts AS P, wp_term_relationships as wtr WHERE w.status = 'publish' AND P.ID = w.wp_post_id AND w.type = '$dtype' AND w.level_of_access = '$level' AND w.list_in_search = '$list'  AND  w.wp_post_id = wtr.object_id AND wtr.term_taxonomy_id IN ('$advancedkeywords') ORDER BY P.post_date_gmt DESC  limit $start, $limit";
            $result = $db->prepare($query);
        } else {
            $query = "SELECT Distinct * FROM wp_resources as w, wp_posts AS P WHERE w.status = 'publish' AND  P.ID = w.wp_post_id AND w.type = '$dtype' AND w.level_of_access = '$level' AND w.list_in_search = '$list' ORDER BY P.post_date_gmt DESC  limit $start, $limit";
            $result = $db->prepare($query);
        }
    } 
...
// if search is 0 and life stage has value and resourcetype has value starts
    if($sort == '0'){
        if($tags !== '0'){
            $query = "SELECT Distinct  l.postid, l.lifestagetype, w.post_title, w.level_of_access, w.list_in_search, w.page_order, w.title, w.type, w.slug, w.wp_post_id 
            FROM life_stage_type AS l, wp_resources AS w, wp_term_relationships as wtr 
            WHERE w.status = 'publish' AND l.lifestagetype = '$lifestage' AND l.postid = w.ID AND w.type = '$dtype' AND  w.wp_post_id = wtr.object_id AND wtr.term_taxonomy_id IN ('$advancedkeywords')  limit $start, $limit"; 
            $result = $db->prepare($query);
        
        } else {
            $query = "SELECT Distinct  l.postid, l.lifestagetype, w.post_title, w.level_of_access, w.list_in_search, w.page_order, w.title, w.type, w.slug, w.wp_post_id 
            FROM life_stage_type AS l, wp_resources AS w
            WHERE w.status = 'publish' AND l.lifestagetype = '$lifestage' AND l.postid = w.ID AND w.type = '$dtype' limit $start, $limit"; 
            $result = $db->prepare($query);
        ...
        if($tags !== '0'){
            $query = "SELECT Distinct  l.postid, l.lifestagetype, w.post_title, w.level_of_access, w.list_in_search, w.page_order, w.title, w.type, w.slug, w.wp_post_id 
            FROM life_stage_type AS l, wp_resources AS w, wp_term_relationships as wtr 
            WHERE w.status = 'publish' AND l.lifestagetype = '$lifestage' AND l.postid = w.ID AND w.type = '$dtype' AND w.page_order = '$pgorder'  AND  w.wp_post_id = wtr.object_id AND wtr.term_taxonomy_id IN ('$advancedkeywords')  limit $start, $limit"; 
            $result = $db->prepare($query);
        
        } else {
            $query = "SELECT Distinct  l.postid, l.lifestagetype, w.post_title, w.level_of_access, w.list_in_search, w.page_order, w.title, w.type, w.slug, w.wp_post_id 
            FROM life_stage_type AS l, wp_resources AS w
            WHERE w.status = 'publish' AND l.lifestagetype = '$lifestage' AND l.postid = w.ID AND w.type = '$dtype' AND w.page_order = '$pgorder'  limit $start, $limit"; 
            $result = $db->prepare($query);
    
        }
    } else if($sort=='views'){
        if($tags !== '0'){
            $query = "SELECT Distinct  l.postid, l.lifestagetype, w.post_title, w.level_of_access, w.list_in_search, w.page_order, w.title, w.type, w.slug, w.wp_post_id, wc.view_count 
            FROM life_stage_type AS l, wp_resources AS w, wp_resources_view_count as wc, wp_term_relationships as wtr 
            WHERE w.status = 'publish' AND l.lifestagetype = '$lifestage' AND l.postid = w.ID AND w.type = '$dtype' AND wc.wp_post_id = w.wp_post_id  AND  w.wp_post_id = wtr.object_id AND w.page_order = '$pgorder' AND wtr.term_taxonomy_id IN ('$advancedkeywords')  ORDER BY wc.view_count DESC limit $start, $limit"; 
            $result = $db->prepare($query);
        ...
        if($tags !== '0'){
            $query = "SELECT Distinct  l.postid, l.lifestagetype, w.post_title, w.level_of_access, w.list_in_search, w.page_order, w.title, w.type, w.slug, w.wp_post_id, P.post_date_gmt
            FROM life_stage_type AS l, wp_resources AS w, wp_posts AS P, wp_term_relationships as wtr 
            WHERE w.status = 'publish' AND l.lifestagetype = '$lifestage' AND P.ID = w.wp_post_id  AND l.postid = w.ID AND w.type = '$dtype' AND P.post_date_gmt  AND  w.wp_post_id = wtr.object_id AND wtr.term_taxonomy_id IN ('$advancedkeywords') DESC limit $start, $limit"; 
            $result = $db->prepare($query);
        } else {
            $query = "SELECT Distinct  l.postid, l.lifestagetype, w.post_title, w.level_of_access, w.list_in_search, w.page_order, w.title, w.type, w.slug, w.wp_post_id, P.post_date_gmt
            FROM life_stage_type AS l, wp_resources AS w, wp_posts AS P
            WHERE w.status = 'publish' AND l.lifestagetype = '$lifestage' AND P.ID = w.wp_post_id  AND l.postid = w.ID AND w.type = '$dtype' AND P.post_date_gmt DESC limit $start, $limit"; 
            $result = $db->prepare($query);
        }
    } 
...
// if search has value and life stage is 0 and resourcetype has value starts
    if($sort == '0'){
        ...
        $where = " WHERE w.type = '$dtype' AND w.status = 'publish' AND P.ID = w.wp_post_id AND w.list_in_search = 'true' AND w.level_of_access = '100'";
        if ($searchQuery != '') {
...
while($row = $result->fetch(PDO::FETCH_ASSOC)){
    $titlen = $row['post_title'];
        $titlesmall = substr_replace($titlen, "...", 30);
    $dvaluen = $row['type'];
    $slug = $row['slug'];
    ...
    $viewUrl = $seo_dvalue.$slug;$variable = resourcesUrl($viewUrl);
        $output .='</div>';
        $output .='<a href="'.$variable.'" target="_self" class="text-view">VIEW</a>';
        $output .='</div>';
