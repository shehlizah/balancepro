    ...
    // life stage is 0 and search is 0 starts
        if($sort == "0"){
            if($tags !='0'){
                ...
                if($tags !='0'){
                    $query = "SELECT DISTINCT * FROM wp_resources as w, wp_posts AS P, wp_resources_view_count as w.status = 'publish' AND wc, wp_term_relationships as wtr WHERE w.type = '$rtypes' AND w.level_of_access = '$level' AND P.ID = w.wp_post_id AND w.list_in_search = '$list' AND  wc.wp_post_id = w.wp_post_id AND  w.wp_post_id = wtr.object_id AND wtr.term_taxonomy_id IN ('$advancedkeywords') ORDER BY P.post_date_gmt DESC  $limit";
                    $stmt = $db->prepare($query);
                } else {
                    $query = "SELECT DISTINCT * FROM wp_resources as w, wp_posts AS P, wp_resources_view_count as wc WHERE w.status = 'publish' AND w.type = '$rtypes' AND w.level_of_access = '$level' AND P.ID = w.wp_post_id AND w.list_in_search = '$list' AND  wc.wp_post_id = w.wp_post_id ORDER BY P.post_date_gmt DESC  $limit";
                    $stmt = $db->prepare($query);
                }
            }
// if search is 0 and life stage is 0 ends
         ...
         if($sort == "0"){
            ...
            WHERE w.status = 'publish' AND l.lifestagetype = '$lifestage' AND l.postid = w.ID AND w.type = '$rtypes' AND wc.wp_post_id = w.wp_post_id  limit 9");
        }else if($sort == "views"){
            ...
            FROM life_stage_type AS l, wp_resources AS w, wp_resources_view_count as wc, wp_term_relationships as wtr
            WHERE w.status = 'publish' AND l.lifestagetype = '$lifestage' AND l.postid = w.ID AND w.type = '$rtypes' AND wc.wp_post_id = w.wp_post_id  AND  w.wp_post_id = wtr.object_id AND wtr.term_taxonomy_id IN ('$advancedkeywords')  order by P.post_date_gmt DESC  limit 9"); 
            
            } else {
                $stmt = $db->prepare("SELECT DISTINCT  l.postid, l.lifestagetype, w.post_title, w.level_of_access, w.list_in_search, w.page_order, w.title, w.type, w.slug, w.wp_post_id, P.post_date_gmt 
            FROM life_stage_type AS l, wp_resources AS w, wp_resources_view_count as wc
            WHERE w.status = 'publish' AND l.lifestagetype = '$lifestage' AND l.postid = w.ID AND w.type = '$rtypes' AND wc.wp_post_id = w.wp_post_id order by P.post_date_gmt DESC  limit 9"); 
...
// if search has value and resourcetype is 0 ends
        ...
        while($rows = $stmt->fetch(PDO::FETCH_ASSOC)){
            ...
            $title = $rows['title'];
            $titlesmall = substr_replace($postname, "...", 30);
            $dvaluen = $rows['type'];
            $slug = $rows['slug'];
