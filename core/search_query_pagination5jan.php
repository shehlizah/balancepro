...
//$unquotedQuery = addslashes($unquotedQuery);
$return_arr = array();
...
// if search is 0 and life stage is 0 and resourcetype is 0 starts
	if($sort == '0'){
		...
		if($tags !='0'){
			...
			$result = $db->prepare("SELECT Distinct * FROM wp_resources as w, wp_posts AS P WHERE w.status = 'publish' AND  w.level_of_access = '$level' AND P.ID = w.wp_post_id AND w.list_in_search = '$list'   ORDER BY P.post_date_gmt DESC ");
		}
		//End
	} 
