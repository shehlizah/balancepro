...
// if search is 0 and life stage is 0 and resourcetype is 0 ends
	...
	if($sort == "0"){
		...
		WHERE w.status = 'publish' AND l.lifestagetype = '$lifestage' AND l.postid = w.ID AND w.type = '$rtypes' AND w.level_of_access = '$level' AND P.ID = w.wp_post_id AND w.list_in_search = '$list' AND w.page_order = '$pgorder' AND wc.wp_post_id = w.wp_post_id ORDER BY P.post_date_gmt DESC";
	    //$limit = 'limt 0, 9';
	    $query = $select;
	     $result = $db->prepare($query);
	     $query1 = $select;
