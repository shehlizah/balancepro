<?php
	...
	function render_search_main_design_lifestage($type, $pager,$lifestage){
		if($pager !=''){
			$pagernew = $pager;
		} else {
			$pagernew = 1;
		    ...
		    $output .='<div class="container background-white">';
			    ...
			    if($resourcetypes == '0'){
					...
					<div class="col-sm-12">
						<div class="input-group">
							<label for="select-all-types">
								<div id="select-all-types" lifestage="'.$lifestage.'" active="0" dvalue="0" pager="'.$pagernew.'" tags="'.$tags.'" search="'.$search.'" sort="'.$sort.'" class="ng-pristine ng-valid">
								<span class="fake-input"></span>
								<span class="fake-label text-center selectalltypes">
								<span class="text">' . __('All Types', 'balance').'</span>
								</span>
				...
				$output .='<!--icons starts -->';
					$output .='<div class="col-xs-6">';
						...
						$output .='<div class="input-group ng-scope">
							...
							$output .='<div class="input-group ng-scope">
							<label for="cb-article">
								<div id="cb-article" class="siderbar-small-category" lifestage="'.$lifestage.'" active="0" dvalue="article" pager="'.$pagernew.'" tags="'.$tags.'" search="'.$search.'" sort="'.$sort.'" tagnames="'.$tagnames.'" inform="rsearchbtn">
					...
					$output .='<div class="col-xs-6">';
						...
						$output .='<div class="input-group ng-scope">
							<label for="cb-calculator">
								<div id="cb-calculator" class="siderbar-small-category" inform="rsearchbtn" lifestage="'.$lifestage.'" active="0" dvalue="calculator" pager="'.$pagernew.'" tags="'.$tags.'" search="'.$search.'" sort="'.$sort.'" tagnames="'.$tagnames.'" >
						...
						if($resourcetypes == 'video'){
								$output .='<div class="input-group ng-scope">
									<label for="cb-video">
										<div id="cb-video" class="siderbar-small-category" inform="rsearchbtn" lifestage="'.$lifestage.'" dvalue="video" pager="'.$pagernew.'" tags="'.$tags.'" search="'.$search.'" sort="'.$sort.'" tagnames="'.$tagnames.'">
						...
						if($resourcetypes == 'newsletter'){
									$output .='<div class="input-group ng-scope">
										<label for="cb-newsletter">
											<div id="cb-newsletter" class="siderbar-small-category" inform="rsearchbtn" lifestage="'.$lifestage.'" dvalue="newsletter" pager="'.$pagernew.'" tags="'.$tags.'" search="'.$search.'" sort="'.$sort.'" tagnames="'.$tagnames.'">
						...
						if($resourcetypes == 'podcast'){
									$output .='<div class="input-group ng-scope">
										<label for="cb-podcast">
											<div id="cb-podcast" class="siderbar-small-category" inform="rsearchbtn" lifestage="'.$lifestage.'" dvalue="podcast" pager="'.$pagernew.'" tags="'.$tags.'" search="'.$search.'" sort="'.$sort.'" tagnames="'.$tagnames.'">
						...
						if($resourcetypes == 'toolkit'){
									    $output .='<div class="input-group ng-scope">
											<label for="cb-toolkit">
											    <div id="cb-toolkit" class="siderbar-small-category" inform="rsearchbtn" lifestage="'.$lifestage.'" dvalue="toolkit" pager="'.$pagernew.'" tags="'.$tags.'" search="'.$search.'" sort="'.$sort.'" tagnames="'.$tagnames.'">
						...
						if($resourcetypes == 'booklet'){
									    $output .='<div class="input-group ng-scope">
											<label for="cb-booklet">
											    <div id="cb-booklet" class="siderbar-small-category" inform="rsearchbtn" lifestage="'.$lifestage.'" dvalue="booklet" pager="'.$pagernew.'" tags="'.$tags.'" search="'.$search.'" sort="'.$sort.'" tagnames="'.$tagnames.'">
...
$maxVisiblePages = 5; // Number of visible pages
$startPage = max(1, $pagernew - 2); // Start from 2 pages before current
$endPage = min($num_of_pages, $pagernew + 2); // End 2 pages after current
...
for ($i = $startPage; $i <= $endPage; $i++) {
    ...
    $output .= '<li class="pg-btn ' . $activeClass . '" style="padding:5px 6px; cursor: pointer;" lifestage="0" typevalue="' . $dvaluen . '" pagerv="' . $i . '" search="0">' . $i . '</li>';
}
...
$lifeurl =  $first."/".$second."/".$third."/".$forth."/";
//if($lifeurl == 'http://www.devxekera.com/life-stages/'){
$resources = 'https://'.$_SERVER['HTTP_HOST'].'/life-stages/';
if($lifeurl == $resources){
	/*echo "its working";
	echo $url2;*/
?>
		...
		$.each(array,function(i, keyword){
			$.each(namearray,function(j, keywordName){
				if(i == j && keyword != "0" && keywordName != "0" && keywordName != ""){
				   ...
				   $(".autocomplete-tag-list").append('<span class="tag ng-scope tgbtn'+keyword+'"><span tagid="'+keyword+'" tagname="'+keywordName+'" class="tagbtn">'+keywordName+'</span><div class="close closetag" tagid="'+keyword+'" url="http://www.devxekera.com/resources/" tagname="'+keywordName+'" tags="'+TagArray+'"></div></span>').before();
				   //console.log(keyword);
				}
			});
		});
