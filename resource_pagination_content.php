...
// $return_arr[] = array("postdata" => $personData);
...
// if search is 0 and life stage is 0 and resourcetype is 0 starts
    if($sort == "0"){
        ...
        if($tags !='0'){
            ...
            $query = "SELECT Distinct post_title, level_of_access, list_in_search, page_order, title, type, slug, wp_post_id FROM wp_resources WHERE status = 'publish' AND level_of_access = '$level' AND list_in_search = '$list' AND page_order = '$pgorder' AND status='publish' limit $start, $limit";
            $result = $db->prepare($query);
         }
