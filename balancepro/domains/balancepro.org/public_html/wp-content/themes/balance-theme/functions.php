...
function save_custom_holiday() {
    ...
    // Fetch existing holidays from wp_postmeta
    $custom_holidays = get_post_meta($post_id, 'custom_holidays_meta', true);
    $custom_holidays = $custom_holidays ? json_decode($custom_holidays, true) : [];
    ...
    update_post_meta($post_id, 'custom_holidays_meta', json_encode($custom_holidays));
    ...
    echo '<button class="remove-holiday">Remove</button>';
    echo '</div>';
    wp_die();
}
...
add_action('wp_ajax_nopriv_remove_custom_holiday', 'remove_custom_holiday');


require_once get_template_directory() . '/partner-reports.php';


// Hook to the 'admin_menu' action to add the custom menu item
add_action( 'admin_menu', 'add_partner_reports_menu' );

function add_partner_reports_menu() {
    // Add the Partner Reports menu page
    add_menu_page(
        'Partner Reports',            // Page Title
        'Partner Reports',            // Menu Title
        'manage_options',             // Capability (who can access this menu)
        'partner-reports',            // Menu Slug (unique identifier for the page)
        'partner_reports_page',       // Callback function to render the page content
        'dashicons-chart-bar',        // Icon (use a dashicon or a custom URL)
        26                           // Position (order of appearance in the sidebar)
    );
}
...
add_action('admin_menu', function () {
    // Remove updates on a specific admin page
    add_action('admin_head', function () {
		$screen = get_current_screen();
		error_log(print_r($screen, true)); // Log the screen object for debugging
        // Replace 'my_custom_page' with the actual page slug or ID
        if ($screen->id === 'toplevel_page_partner-reports') {
            // Remove the update notification bar
			remove_action('admin_notices', 'update_nag', 3);

			// Remove the WordPress version from the footer
			add_filter('update_footer', '__return_empty_string', 11);
	
			// Remove the "Thank you for using WordPress" footer text
			add_filter('admin_footer_text', '__return_empty_string');
	
            // Hide the updates menu from the sidebar
            add_action('admin_menu', function () {
                remove_menu_page('update-core.php');
            });

            // Remove the updates bubble in the admin bar
