...
// if search has value starts
	    ...
	    $select = "SELECT Distinct (w.title = '{$unquotedQuery}') AS title_match, match (w.html, w.title) against ('{$searchQuery}') AS relevancy, (w.post_title LIKE '%{$unquotedQuery}%') AS title_rough_match, w.post_title, w.level_of_access, w.list_in_search, w.page_order, w.title, w.type, w.slug, w.wp_post_id, wc.view_count";
	     //$from = " FROM wp_resources as w, wp_posts AS P, wp_resources_view_count as wc";
	     $from = " FROM wp_posts AS P, wp_resources as w, wp_resources_view_count as wc ";
		 //$ljoin = ", wp_resources as w left join wp_resources_view_count as wc on wc.wp_post_id = w.wp_post_id ";	
		 
		 $where = " WHERE w.status = 'publish' AND P.ID = w.wp_post_id AND w.list_in_search = 'true' AND w.level_of_access = '100' AND w.page_order='1' AND wc.wp_post_id = w.wp_post_id ";
		 //$where = " WHERE w.status = 'publish' AND P.ID = w.wp_post_id AND w.list_in_search = 'true' AND w.level_of_access = '100' AND wc.wp_post_id = w.wp_post_id";
	     if ($searchQuery != '') {
	     	$where .= " and (match (w.html, w.title) against ('{$searchQuery}' in boolean mode) or match (P.post_excerpt) against ('{$searchQuery}' in boolean mode))";
	     }
...
// if search is 0 and life stage is 0 and resourcetype is 0 starts
	if($sort == "0"){
		...
		// echo "5";
	    $select = "SELECT Distinct * FROM wp_resources as w, wp_posts AS P, wp_resources_view_count as wc WHERE w.status = 'publish' AND w.level_of_access = '$level' AND P.ID = w.wp_post_id AND w.list_in_search = '$list' AND w.page_order = '$pgorder' AND wc.wp_post_id = w.wp_post_id ORDER BY wc.view_count DESC";
	    //$limit = 'limt 0, 9';
	    $query = $select;
	     $result = $db->prepare($query);
	     $query1 = $select;
	    ...
	    $select = "SELECT Distinct * FROM wp_resources as w, wp_posts AS P, wp_resources_view_count as wc WHERE w.status = 'publish' AND w.level_of_access = '$level' AND P.ID = w.wp_post_id AND w.list_in_search = '$list' AND w.page_order = '$pgorder' AND wc.wp_post_id = w.wp_post_id ORDER BY P.post_date_gmt DESC";
	    //$limit = 'limt 0, 9';
	    $query = $select;
	     $result = $db->prepare($query);
	     $query1 = $select;
...
// if search is 0 and life stage is 0 and resourcetype is 0 ends
	...
	if($sort == "0"){
	    ...
	    $select = "SELECT Distinct l.postid, l.lifestagetype, w.post_title, w.level_of_access, w.list_in_search, w.page_order, w.title, w.type, w.slug, w.wp_post_id, (w.title = '{$unquotedQuery}') AS title_match 
		FROM life_stage_type AS l, wp_resources AS w, wp_resources_view_count as wc
		WHERE w.status = 'publish' AND title LIKE '%{$unquotedQuery}%' AND l.lifestagetype = '$lifestage' AND l.postid = w.ID AND w.type = '$rtypes' AND list_in_search = 'true' AND level_of_access = '100 AND w.page_order = '$pgorder'' AND wc.wp_post_id = w.wp_post_id order by title_match DESC";
	    //$limit = 'limt 0, 9';
	    $query = $select;
	     $result = $db->prepare($query);
	     $query1 = $select;
		...
		FROM life_stage_type AS l, wp_resources AS w, wp_resources_view_count as wc, wp_posts AS P
		WHERE w.status = 'publish' AND title LIKE '%{$unquotedQuery}%' AND l.lifestagetype = '$lifestage' AND P.ID = w.wp_post_id AND l.postid = w.ID AND w.type = '$rtypes' AND list_in_search = 'true' AND w.page_order = '$pgorder' AND level_of_access = '100' AND wc.wp_post_id = w.wp_post_id order by  P.post_date_gmt DESC";
	    //$limit = 'limt 0, 9';
	    $query = $select;
	     $result = $db->prepare($query);
	     $query1 = $select;
...
$return_arr['query1'] = $query;


			
 //check 
	$checkn->execute();
	$countthem = $checkn->rowCount();
	//now count row 
	$checkcountn = $checkn->rowCount();
	/*$return_arr['query1'] = $query1;
