    ...
    $viewUrl = $seo_dvalue.$slug;
        ...
        while($resultn = $resultss->fetch(PDO::FETCH_ASSOC)) {
           ...
           while($resultnn = $gettagname->fetch(PDO::FETCH_ASSOC)){
               $output .='<span class="tag ng-binding ng-scope tag-click" url="'.$escaped_url.'" tagname="'.$resultnn['name'].'" pager="'.$pager.'" type="'.$type.'" dvalue="'.$tagid.'" tagid="'.$tagid.'" tags="'.$tagid.'" sort="'.$sort.'">';
