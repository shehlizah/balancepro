...
// Fetch custom holidays from the wp_postmeta table
$holidays_json = get_post_meta($post_id, 'custom_holidays_meta', true);
$custom_holidays = $holidays_json ? json_decode($holidays_json, true) : [];
  ...
  function is_today_a_holiday($holidays, $custom_holidays) {
    ...
    if (isset($holidays) && is_array($holidays)) {
        foreach ($holidays as $holiday) {
            if ($holiday == $current_date|| $holiday==$current_date_custom) {
  ...
  $output .= '<div style="margin-top: 20px;">';
    ...
    function toggleTimeSettings(day, isEnabled) {
        ["start", "start_minutes", "start_ap", "end", "end_minutes", "end_ap"].forEach(function(suffix) {
            var dropdown = document.querySelector("[name=\'timesettings[" + day + "_" + suffix + "]\']");
            dropdown.disabled = !isEnabled; // Disable or enable based on toggle state
            dropdown.style.border = ""; // Clear border style when toggled
        });
    }
    ...
    days.forEach(function(day) {
        ...
        var isChecked = toggleCheckbox.checked; // Check if the toggle is checked
    ...
    document.querySelectorAll(".toggleCheck").forEach(function(checkbox) {
    ...
    function checkAllValidations() {
        var allValid = days.every(function(day) {
            return validateTime(day);
        });
        updateButton.disabled = !allValid; // Enable if all are valid, otherwise disable
    }
...
// }
      ...
      // Add CSS for greyed-out disabled button
        ...
        // Set the minimum date to today
            ...
            let existingHolidays = ' . json_encode($custom_holidays) . ';
            ...
            function holidayExists(name, date) {
                for (let i = 0; i < existingHolidays.length; i++) {
                    if (existingHolidays[i].name.toLowerCase() === name.toLowerCase() || 
                        existingHolidays[i].date === date) {
                        return true;
                    }
                }
                return false;
            ...
            $("#holidayForm").on("submit", function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $("#holidayFormContainer").hide(); // Hide the form
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            existingHolidays.splice(index, 1); // Remove from array
                        }
                    }
                });
            });
...
// END HOLIDAY 
  ...
  $output .= '<label><p class="headings">Message Text:</p>';
   ...
   if (is_today_a_holiday($holidays, $custom_holidays)) {
    $chat_message = '<p> Due to a holiday today, BALANCE chat is currently unavailable.</p>';
    }
...
//       endTime.hour = (endTime.ampm === "PM" && endTime.hour !== 12) ? endTime.hour + 12 : endTime.hour;
